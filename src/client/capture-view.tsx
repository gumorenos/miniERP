...
export function CaptureView({ data, onCancel, onChanged, onCreated }: {
  ...
  return <div className="stack">
    ...
    {!draft && recentDrafts.length > 0 && <section><div className="section-heading"><div><p className="eyebrow">Bandeja de captura</p><h2>Borradores pendientes</h2><p className="muted">Puedes retomarlos sin volver a pegar el mensaje.</p></div></div><div className="list">{recentDrafts.map((item) => <button type="button" className="row" key={item.id} onClick={() => { pendingSourceMessageId.current = null; setSuccess(""); setError(""); setMessage(item.rawText); hydrateDraft(item); }}><span><strong>{captureIntentLabel(item.intent)}</strong><small>{item.rawText}</small></span><span><b>Revisar</b></span></button>)}</div></section>}
