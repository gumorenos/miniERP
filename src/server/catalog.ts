...
const productSchema = z.object({
  ...
  baseSalePrice: z.coerce.number().positive(),
  leadTimeDays: z.coerce.number().int().min(0).max(365).default(25),
  defaultFabricMaterialId: nullableUuid,
  defaultFabricQtyMeters: z.coerce.number().positive().optional().nullable(),
  defaultClosureMaterialId: nullableUuid,
  defaultClosureQty: z.coerce.number().positive().optional().nullable(),
  ...
  defaultPackagingMaterialId: nullableUuid,
  defaultPackagingQty: z.coerce.number().positive().optional().nullable(),
  xlAdjustment: z.coerce.number().nonnegative().default(0),
  xxlAdjustment: z.coerce.number().nonnegative().default(0),
...
async function requireBusinessMaterial(businessId: string, id: string, expectedCategory?: string) {
  ...
  if (expectedCategory && material.category !== expectedCategory) return null;
  return material;
}

async function currentQuantity(materialId: string) {
  const [row] = await db
    .select({ quantity: sql<string>`coalesce(sum(${stockMovements.quantitySigned}), 0)` })
    .from(stockMovements)
...
export async function handleCatalogMutation(request: Request, user: AuthUser): Promise<Response> {
    ...
    const result = await db.transaction(async (tx) => {
      const [material] = await tx
        ...
        .values({
          ...
          minimumStock: body.minimumStock == null ? null : String(body.minimumStock)
        })
        .returning();
      ...
      if (body.initialQuantity > 0) {
        await tx.insert(stockMovements).values({
          ...
          type: "INITIAL_STOCK",
          quantitySigned: String(body.initialQuantity),
  ...
  if (stockMatch) {
    ...
    await db.insert(stockMovements).values({
      ...
      materialId,
      type: "PURCHASE",
      quantitySigned: String(parsed.data.quantity),
      unitCost: parsed.data.unitCost == null ? null : String(parsed.data.unitCost),
      notes: parsed.data.notes || "Entrada de stock"
    });

    return json({ materialId, currentQuantity: await currentQuantity(materialId) }, 201);
  }
    ...
    const product = await db.transaction(async (tx) => {
      const [created] = await tx
        ...
        .values({
          ...
          name: body.name,
          type: body.type,
          baseSalePrice: String(body.baseSalePrice),
          leadTimeDays: body.leadTimeDays,
          defaultFabricMaterialId: fabricId,
          defaultFabricQtyMeters: body.defaultFabricQtyMeters == null ? null : String(body.defaultFabricQtyMeters),
          defaultClosureMaterialId: closureId,
          defaultClosureQty: closureId && body.defaultClosureQty != null ? String(body.defaultClosureQty) : null,
          defaultEmbroideryCost: body.defaultEmbroideryCost == null ? null : String(body.defaultEmbroideryCost),
          defaultOwnLaborCost: body.defaultOwnLaborCost == null ? null : String(body.defaultOwnLaborCost),
          defaultPackagingMaterialId: packagingId,
          defaultPackagingQty: packagingId && body.defaultPackagingQty != null ? String(body.defaultPackagingQty) : null,
      ...
      await tx.insert(productSizePrices).values(
        sizes.map((size) => ({
          ...
          size,
          priceAdjustment: String(size === "XL" ? body.xlAdjustment : size === "XXL" ? body.xxlAdjustment : 0)
        }))
      );
      return created;
    });

    const sizePrices = await db.select().from(productSizePrices).where(eq(productSizePrices.productId, product.id));
    return json({ ...product, sizePrices }, 201);
  }
