  ...
  const rows = await db.execute(sql`
    select p.id, p.purchase_date, p.supplier_name, p.total_amount, p.payment_method, p.notes,
           coalesce(json_agg(json_build_object(
             'id', pl.id, 'materialId', pl.material_id, 'materialName', m.name,
             'quantity', pl.quantity, 'totalCost', pl.total_cost, 'unitCost', pl.unit_cost
           ) order by m.name) filter (where pl.id is not null), '[]'::json) as lines
    from purchases p
    ...
    where p.business_id = ${user.businessId}::uuid
      and not exists (
        select 1 from deleted_records d where d.business_id = p.business_id and d.entity_type = 'PURCHASE' and d.entity_id = p.id
      )
    group by p.id
    order by p.purchase_date desc, p.created_at desc
  `);
  return json({ rows: resultRows(rows.rows) });
}

function resultRows(rows: Record<string, unknown>[]) {
  return rows.map((row) => ({
...
export async function createPurchase(request: Request, user: AuthUser) {
  ...
  const id = await db.transaction(async (tx) => {
    const [purchase] = await tx.insert(purchases).values({
      ...
      purchaseDate: parsed.data.purchaseDate ?? limaBusinessDate(),
    ...
    for (const line of parsed.data.lines) {
      ...
      const [createdLine] = await tx.insert(purchaseLines).values({
        purchaseId: purchase.id, materialId: line.materialId, quantity: String(line.quantity), totalCost: String(line.totalCost), unitCost: String(unitCost)
      }).returning();
      await tx.insert(stockMovements).values({
        businessId: user.businessId, materialId: line.materialId, type: "PURCHASE", quantitySigned: String(line.quantity), unitCost: String(unitCost), purchaseLineId: createdLine.id,
...
export async function updatePurchase(request: Request, user: AuthUser) {
  ...
  if (await isArchived(user.businessId, "PURCHASE" as never, parsed.data.id)) return json({ error: "La compra fue borrada" }, 409);
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.update(purchases).set({
      ...
      ...(parsed.data.supplierName !== undefined ? { supplierName: parsed.data.supplierName || null } : {}),
...
export async function archivePurchase(request: Request, user: AuthUser) {
  ...
  if (lineIds.length) {
    ...
    for (const line of current.lines) {
      ...
      const after = await currentStock(line.materialId) - Number(movement.quantitySigned);
      if (after < -0.0001) return json({ error: "No se puede borrar esta compra porque el material ya fue consumido y el stock quedaría negativo" }, 409);
    }
  }
  await db.transaction(async (tx) => {
    await tx.execute(sql`
      insert into deleted_records (business_id, entity_type, entity_id, snapshot)
      values (${user.businessId}::uuid, 'PURCHASE', ${current.id}::uuid, ${JSON.stringify(current)}::jsonb)
