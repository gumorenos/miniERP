...
import { materials, purchaseLines, purchases, stockMovements, suppliers } from "../db/schema";
...
const createSchema = z.object({
  action: z.literal("create").optional(), purchaseDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), supplierId: z.string().uuid().optional().nullable(), supplierName: z.string().trim().max(160).optional().nullable(),
...
const updateSchema = z.object({
  action: z.literal("update"), id: z.string().uuid(), purchaseDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(), supplierId: z.string().uuid().optional().nullable(), supplierName: z.string().trim().max(160).optional().nullable(),
...
async function supplierForBusiness(businessId: string, id: string) {
  if (await isArchived(businessId, "SUPPLIER", id)) return null;
  const [row] = await db.select().from(suppliers).where(and(eq(suppliers.id, id), eq(suppliers.businessId, businessId), eq(suppliers.active, true))).limit(1);
  return row ?? null;
}
...
function resultRows(rows: readonly Record<string, unknown>[]) {
  return rows.map((row) => ({
    id: String(row.id), purchaseDate: String(row.purchase_date), supplierId: row.supplier_id == null ? null : String(row.supplier_id), supplierName: row.supplier_name == null ? null : String(row.supplier_name),
...
export async function listPurchases(user: AuthUser) {
  const rows = await db.execute(sql`
    select p.id, p.purchase_date, p.supplier_id, p.supplier_name, p.total_amount, p.payment_method, p.notes,
...
export async function createPurchase(request: Request, user: AuthUser) {
  ...
  const supplier = parsed.data.supplierId ? await supplierForBusiness(user.businessId, parsed.data.supplierId) : null;
  if (parsed.data.supplierId && !supplier) return json({ error: "El proveedor no pertenece al negocio o fue borrado" }, 400);
  const supplierName = supplier?.name ?? (parsed.data.supplierName?.trim() || null);
  ...
  const id = await db.transaction(async (tx) => {
    const [purchase] = await tx.insert(purchases).values({ businessId:user.businessId,purchaseDate:parsed.data.purchaseDate??limaBusinessDate(),supplierId:supplier?.id??null,supplierName,totalAmount:String(total),paymentMethod:parsed.data.paymentMethod||null,notes:parsed.data.notes||null }).returning();
    for (const line of parsed.data.lines) {
