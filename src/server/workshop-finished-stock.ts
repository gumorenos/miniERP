...
export async function listFinishedStock(user: AuthUser) {
  const [balances, movementRows, archivedRows] = await Promise.all([
    ...
    db.execute(sql`select entity_type,entity_id::text as id from deleted_records where business_id=${user.businessId}::uuid and entity_type in ('FINISHED_STOCK_MOVEMENT','PRODUCT')`)
  ]);
  const hiddenMovements = new Set(archivedRows.rows.filter((row) => String(row.entity_type) === "FINISHED_STOCK_MOVEMENT").map((row) => String(row.id)));
  const hiddenProducts = new Set(archivedRows.rows.filter((row) => String(row.entity_type) === "PRODUCT").map((row) => String(row.id)));
  return json({ balances: balances.rows.map((row) => ({ productId:String(row.product_id),productName:String(row.product_name),size:String(row.size),color:String(row.color),quantity:Number(row.quantity) })), movements: movementRows.filter((row) => !hiddenMovements.has(row.id) && !hiddenProducts.has(row.productId)).map((row) => ({ ...row,unitCost:row.unitCost==null?null:Number(row.unitCost) })) });
}
...
export async function saveFinishedStock(request: Request, user: AuthUser) {
  ...
  if (raw?.action === "update") {
    ...
    if(await isArchived(user.businessId,"PRODUCT",current.productId))return json({error:"El producto fue borrado y su historial de stock ya no se puede modificar"},409);
