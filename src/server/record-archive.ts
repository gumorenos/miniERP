...
async function saveArchive(businessId: string, entityType: ArchiveEntityType, entityId: string, snapshot: unknown) {
  await db.execute(sql`
    ...
    values (${businessId}::uuid, ${entityType}, ${entityId}::uuid, ${JSON.stringify(snapshot)}::jsonb)
...
export async function isArchived(businessId: string, entityType: ArchiveEntityType, entityId: string) {
  const result = await db.execute(sql`
    ...
    where business_id = ${businessId}::uuid and entity_type = ${entityType} and entity_id = ${entityId}::uuid
    limit 1
  `);
...
export async function archiveRecord(request: Request, user: AuthUser) {
  ...
  if (entityType === "CUSTOMER") {
    ...
    const activeOrders = await db.select({ id: orders.id }).from(orders).where(and(eq(orders.businessId, user.businessId), eq(orders.customerId, row.id), sql`${orders.status} not in ('DELIVERED', 'CLOSED', 'CANCELLED')`)).limit(1);
    if (activeOrders.length) return json({ error: "El cliente tiene pedidos activos. Ciérralos o cancélalos antes de borrarlo." }, 409);
    await saveArchive(user.businessId, entityType, id, row);
    return json({ ok: true });
  }
  ...
  if (entityType === "MATERIAL") {
    ...
    const activeOrderUse = await db.execute(sql`
      ...
      where o.business_id = ${user.businessId}::uuid
        and oi.fabric_material_id = ${row.id}::uuid
        and o.status not in ('DELIVERED', 'CLOSED', 'CANCELLED')
      limit 1
    `);
    if (activeOrderUse.rows.length) return json({ error: "Este material todavía está reservado por un pedido activo y no puede borrarse." }, 409);
    await saveArchive(user.businessId, entityType, id, row);
    return json({ ok: true });
  }
  ...
  if (entityType === "PAYMENT") {
    ...
    await db.transaction(async (tx) => {
      await tx.execute(sql`
        insert into deleted_records (business_id, entity_type, entity_id, snapshot)
        values (${user.businessId}::uuid, ${entityType}, ${id}::uuid, ${JSON.stringify(row)}::jsonb)
        on conflict (business_id, entity_type, entity_id) do nothing
      `);
  ...
  if (!["ORDER_RECEIVED", "MATERIAL_PENDING", "READY_TO_CUT", "CANCELLED"].includes(order.status)) {
    return json({ error: "Este pedido ya afectó el flujo de producción. Cancélalo para detenerlo, pero conserva su historial." }, 409);
  }
  ...
  if (itemIds.length) {
    ...
    const jobs = await db.select({ id: embroideryJobs.id }).from(embroideryJobs).where(inArray(embroideryJobs.orderItemId, itemIds)).limit(1);
    if (jobs.length) return json({ error: "El pedido ya tiene trabajo de bordado y no puede borrarse." }, 409);
  }

  await db.transaction(async (tx) => {
    await tx.execute(sql`
      insert into deleted_records (business_id, entity_type, entity_id, snapshot)
      values (${user.businessId}::uuid, ${entityType}, ${id}::uuid, ${JSON.stringify({ order, items, payments: paymentRows })}::jsonb)
      on conflict (business_id, entity_type, entity_id) do nothing
    `);
    await tx.update(orders).set({ status: "CANCELLED", agreedTotalPrice: "0", updatedAt: new Date() }).where(eq(orders.id, order.id));
    if (itemIds.length) {
      await tx.update(orderItems).set({ agreedUnitPrice: "0", estimatedMaterialCost: "0", estimatedOwnLaborCost: "0", estimatedPackagingCost: "0", otherEstimatedDirectCost: "0" }).where(inArray(orderItems.id, itemIds));
    }
  });
  return json({ ok: true });
}
