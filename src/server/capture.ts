...
import { captureDrafts, customers, productSizePrices, products, materials } from "../db/schema";
...
async function activeCatalog(businessId: string) {
  const [customerRows, productRows, materialRows, archivedRows] = await Promise.all([
    ...
    db.execute(sql`select entity_type, entity_id from deleted_records where business_id=${businessId}::uuid and entity_type in ('CUSTOMER','PRODUCT','MATERIAL')`)
  ]);
